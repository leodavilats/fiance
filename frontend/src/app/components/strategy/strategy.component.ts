...
@Component({
  ...
  template: `
    <!-- Tabs de navegação -->
    <div class="flex gap-1 mb-5 p-1 rounded-xl bg-panel border border-border w-fit flex-wrap">
      <button
        type="button"
        class="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all"
        [class.bg-accent]="activeTab() === 'analisar'"
        [class.text-white]="activeTab() === 'analisar'"
        [class.text-muted]="activeTab() !== 'analisar'"
        [class.hover:text-tx]="activeTab() !== 'analisar'"
        (click)="activeTab.set('analisar')"
      >
        <lucide-icon name="search" size="15"></lucide-icon> Analisar Ativo
      </button>
      <button
        type="button"
        class="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all"
        [class.bg-accent]="activeTab() === 'ajuste'"
        [class.text-white]="activeTab() === 'ajuste'"
        [class.text-muted]="activeTab() !== 'ajuste'"
        [class.hover:text-tx]="activeTab() !== 'ajuste'"
        (click)="activeTab.set('ajuste')"
      >
        <lucide-icon name="wand-sparkles" size="15"></lucide-icon> Ajuste de Carteira
      </button>
      <button
        type="button"
        class="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all"
        [class.bg-accent]="activeTab() === 'renda_fixa'"
        [class.text-white]="activeTab() === 'renda_fixa'"
        [class.text-muted]="activeTab() !== 'renda_fixa'"
        [class.hover:text-tx]="activeTab() !== 'renda_fixa'"
        (click)="activeTab.set('renda_fixa')"
      >
        <lucide-icon name="landmark" size="15"></lucide-icon> Renda Fixa
      </button>
    </div>
        ...
        <div class="p-5 rounded-lg bg-panel border border-border">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
            ...
            <div class="p-4 rounded-lg bg-bg-2 border border-border good">
              ...
              <div class="text-xl font-bold text-tx">
                {{
                  a.fair_price.consensus != null ? (a.fair_price.consensus | number: '1.2-2') : '—'
                }}
              </div>
            </div>
            <div
                    ...
                    @if (sug.transaction_cost) {
                      ...
                      >
                        <div class="flex items-center gap-1.5 text-orange-400 font-medium mb-1">
                          <lucide-icon name="receipt" size="13"></lucide-icon> IR estimado na venda
                        </div>
      ...
      @if (referenceRates()) {
        ...
        >
          ...
          <span class="font-medium text-tx"
            >CDI: <strong class="text-accent">{{ referenceRates()!.cdi_anual }}% a.a.</strong></span
          >
          <span class="font-medium text-tx"
            >Selic:
