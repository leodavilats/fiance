  ...
  template: `
    <div class="p-5 rounded-lg bg-panel border border-border">
      <h2 class="flex items-center gap-2 text-xl font-bold m-0 mb-3 text-tx">
        <lucide-icon name="target" size="18"></lucide-icon> Oportunidades
      </h2>
      <p class="text-sm text-muted mb-4">
        Universo curado + sua watchlist, rankeado por mérito individual. Use filtros para refinar.
      </p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <input
            ...
            [class.border-accent]="filterText()"
            [class.border-border]="!filterText()"
            [class.bg-accent]="filterText()"
            [class.bg-opacity-10]="filterText()"
            [value]="filterText()"
        ...
        <div>
          ...
          <select
            class="w-full px-3 py-2 rounded-lg bg-bg border text-tx text-sm focus:outline-none focus:ring-2 focus:ring-accent cursor-pointer transition-all"
            [class.border-accent]="filterSector()"
            [class.border-border]="!filterSector()"
            [class.bg-accent]="filterSector()"
            [class.bg-opacity-10]="filterSector()"
            [class.font-semibold]="filterSector()"
            [value]="filterSector()"
            (change)="filterSector.set($any($event.target).value)"
          >
            <option value="">Todos</option>
            @for (s of availableSectors(); track s) {
              <option [value]="s">{{ ui.translateSector(s) }}</option>
            }
          </select>
        </div>
        <div>
          ...
          <select
            class="w-full px-3 py-2 rounded-lg bg-bg border text-tx text-sm focus:outline-none focus:ring-2 focus:ring-accent cursor-pointer transition-all"
            [class.border-accent]="sortKey() !== 'score' || sortOrder() !== 'desc'"
            [class.border-border]="sortKey() === 'score' && sortOrder() === 'desc'"
            [class.bg-accent]="sortKey() !== 'score' || sortOrder() !== 'desc'"
            [class.bg-opacity-10]="sortKey() !== 'score' || sortOrder() !== 'desc'"
            [class.font-semibold]="sortKey() !== 'score' || sortOrder() !== 'desc'"
            [value]="sortKey()"
            (change)="changeSortKey($any($event.target).value)"
          >
            <option value="score">
            ...
            <option value="mos">
              Margem de segurança
              {{ sortKey() === 'mos' ? (sortOrder() === 'desc' ? '↓' : '↑') : '' }}
            </option>
            <option value="dy">
              Dividend Yield {{ sortKey() === 'dy' ? (sortOrder() === 'desc' ? '↓' : '↑') : '' }}
            </option>
            <option value="price">
              Preço {{ sortKey() === 'price' ? (sortOrder() === 'desc' ? '↓' : '↑') : '' }}
            </option>
          </select>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          ...
          <div class="flex gap-1 flex-wrap">
            <button
              type="button"
              class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
              [class.bg-accent]="filterType() === 'all'"
              [class.border-accent]="filterType() === 'all'"
              [class.text-white]="filterType() === 'all'"
              [class.bg-panel-2]="filterType() !== 'all'"
              [class.border-border]="filterType() !== 'all'"
              [class.text-tx]="filterType() !== 'all'"
              [class.hover:bg-panel]="filterType() !== 'all'"
            ...
            <button
              type="button"
              class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
              [class.bg-accent]="filterType() === 'br_stock'"
              [class.border-accent]="filterType() === 'br_stock'"
              [class.text-white]="filterType() === 'br_stock'"
              [class.bg-panel-2]="filterType() !== 'br_stock'"
              [class.border-border]="filterType() !== 'br_stock'"
              [class.text-tx]="filterType() !== 'br_stock'"
              [class.hover:bg-panel]="filterType() !== 'br_stock'"
              (click)="filterType.set('br_stock')"
            >
              BR
            </button>
            <button
              type="button"
              class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
              [class.bg-accent]="filterType() === 'fii'"
              [class.border-accent]="filterType() === 'fii'"
              [class.text-white]="filterType() === 'fii'"
              [class.bg-panel-2]="filterType() !== 'fii'"
              [class.border-border]="filterType() !== 'fii'"
              [class.text-tx]="filterType() !== 'fii'"
              [class.hover:bg-panel]="filterType() !== 'fii'"
              (click)="filterType.set('fii')"
            >
              FII
            </button>
            <button
              type="button"
              class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
              [class.bg-accent]="filterType() === 'us_stock'"
              [class.border-accent]="filterType() === 'us_stock'"
              [class.text-white]="filterType() === 'us_stock'"
              [class.bg-panel-2]="filterType() !== 'us_stock'"
              [class.border-border]="filterType() !== 'us_stock'"
              [class.text-tx]="filterType() !== 'us_stock'"
              [class.hover:bg-panel]="filterType() !== 'us_stock'"
              (click)="filterType.set('us_stock')"
            >
              EUA
            </button>
            <button
              type="button"
              class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
              [class.bg-accent]="filterType() === 'crypto'"
              [class.border-accent]="filterType() === 'crypto'"
              [class.text-white]="filterType() === 'crypto'"
              [class.bg-panel-2]="filterType() !== 'crypto'"
              [class.border-border]="filterType() !== 'crypto'"
              [class.text-tx]="filterType() !== 'crypto'"
              [class.hover:bg-panel]="filterType() !== 'crypto'"
              (click)="filterType.set('crypto')"
            >
              Cripto
            </button>
          </div>
        ...
        <div>
          ...
          <div class="flex gap-1 flex-wrap">
            @for (cat of CATEGORY_FILTERS; track cat.key) {
              <button
                type="button"
                class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer border transition-all"
                [class.bg-accent]="filterCategory() === cat.key"
                [class.border-accent]="filterCategory() === cat.key"
                [class.text-white]="filterCategory() === cat.key"
                [class.bg-panel-2]="filterCategory() !== cat.key"
                [class.border-border]="filterCategory() !== cat.key"
                [class.text-tx]="filterCategory() !== cat.key"
        ...
        <div class="flex flex-col gap-2">
          <label
            class="flex items-center gap-2 text-sm cursor-pointer transition-all"
            [class.text-tx]="!includeHeld()"
            [class.text-accent]="includeHeld()"
            [class.font-semibold]="includeHeld()"
          >
            <input
              type="checkbox"
              class="w-4 h-4 rounded border-border accent-accent cursor-pointer"
              [checked]="includeHeld()"
              (change)="includeHeld.set($any($event.target).checked)"
            />
            Incluir já em carteira
          </label>
          <label
            class="flex items-center gap-2 text-sm cursor-pointer transition-all"
            [class.text-tx]="!onlyInteresting()"
            [class.text-accent]="onlyInteresting()"
            [class.font-semibold]="onlyInteresting()"
          >
            <input
              type="checkbox"
              class="w-4 h-4 rounded border-border accent-accent cursor-pointer"
              [checked]="onlyInteresting()"
              (change)="onlyInteresting.set($any($event.target).checked)"
            />
    ...
    @if (opps()) {
      <div class="p-5 rounded-lg bg-panel border border-border">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold m-0 text-tx">{{ totalItems() }} oportunidade(s)</h2>
          <div class="text-sm text-muted">Página {{ currentPage() }} de {{ totalPages() }}</div>
        </div>
        <div class="flex flex-col gap-3">
          @for (o of opps(); track o.ticker) {
            <div
              class="flex justify-between items-start gap-4 p-3 rounded-lg bg-panel-2 border hover:shadow-lg transition-shadow"
            ...
            >
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap mb-1">
                  @if (o.is_interesting) {
                    <span
                      class="flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold bg-accent text-white"
                    >
                  ...
                  <span class="tag" *ngIf="o.in_watchlist"
                    ><lucide-icon name="star" size="11"></lucide-icon> watchlist</span
                  >
                </div>
                @if (o.name && o.name !== o.ticker) {
                  <div class="text-sm text-muted mb-2">
                    {{ o.name }}
                    @if (o.sector) {
                      · {{ ui.translateSector(o.sector) }}
                    }
                  </div>
                } @else if (o.sector) {
                  <div class="text-sm text-muted mb-2">{{ ui.translateSector(o.sector) }}</div>
                }
                <div class="flex items-center gap-2 flex-wrap text-xs">
                  ...
                  <span *ngIf="o.margin_of_safety != null" class="text-muted"
                    >MS: <strong>{{ o.margin_of_safety * 100 | number: '1.0-0' }}%</strong></span
                  >
              ...
              <div class="text-right flex-shrink-0">
                ...
                <div class="flex gap-1.5 mt-1.5">
                  <button
                    class="flex items-center gap-1 px-2 py-1 rounded text-xs font-medium cursor-pointer bg-panel-2 border border-border text-tx hover:bg-panel transition-all"
                    (click)="openDipAnalysis(o.ticker)"
                  >
    ...
    @if (showDipPanel()) {
      ...
      <div
        class="fixed inset-0 md:left-auto md:w-full md:max-w-2xl bg-panel shadow-2xl z-50 flex flex-col"
      >
        <div
          class="flex justify-between items-start gap-4 p-5 bg-panel border-b border-border flex-shrink-0"
        >
          <div>
            @if (dipPanelResult(); as d) {
              <div class="text-lg font-bold text-tx">
                ...
                @if (d.name && d.name !== d.symbol) {
                  <span class="text-muted font-normal text-sm">{{ d.name }}</span>
                }
              </div>
          ...
          <button
            class="w-9 h-9 grid place-items-center rounded-lg cursor-pointer bg-panel-2 border border-border text-tx hover:bg-panel transition-opacity"
            (click)="closeDipPanel()"
          >
        ...
        <div class="flex-1 overflow-y-auto">
          @if (dipPanelResult(); as d) {
            <div class="p-5">
              <div
                class="dip-verdict mb-5 p-4 rounded-lg border-2"
                [class]="'dip-' + d.verdict.toLowerCase()"
              >
                <div class="flex items-center gap-2 text-lg font-bold mb-2">
                  <lucide-icon
                    [name]="
                      d.verdict === 'OPORTUNIDADE'
                        ? 'circle-check'
                        : d.verdict === 'ARMADILHA'
                          ? 'circle-x'
                          : 'circle-minus'
                ...
                <div class="text-sm mt-2">
                  Score: <strong>{{ d.dip_score | number: '1.0-0' }}</strong
                  >/100
                </div>
                <div class="text-sm text-muted">
                  Confiança: {{ d.confidence * 100 | number: '1.0-0' }}%
                </div>
              </div>
              ...
              <div class="mb-4">
                <h3 class="text-sm font-semibold mb-2 text-tx flex items-center gap-2">
                  ...
                  @if (d.news_ai_summary) {
                    <span
                      class="px-2 py-0.5 text-[10px] rounded-full bg-accent/20 text-accent font-medium"
                      >IA</span
                    >
                  }
                </h3>
                ...
                @if (d.news_ai_summary) {
                  <div
                    class="p-3 rounded-lg bg-gradient-to-br from-accent/5 to-accent/10 border border-accent/20 mb-3"
                  >
                    ...
                    @if (d.news_key_topics && d.news_key_topics.length > 0) {
                      <div class="flex items-start gap-2 mt-2.5">
                        <lucide-icon
                          name="list"
                          size="11"
                          class="text-muted flex-shrink-0 mt-0.5"
                        ></lucide-icon>
                        <div class="flex gap-1.5 flex-wrap">
                          @for (topic of d.news_key_topics; track topic) {
                            <span
                              class="px-2 py-0.5 rounded-full text-[10px] bg-panel-2 border border-border text-muted"
                              >{{ topic }}</span
                            >
                          }
                        </div>
                ...
                @if (d.news.length === 0) {
                  ...
                  <div class="flex flex-col gap-2">
                    @for (n of d.news; track n.url) {
                      <div
                        class="p-2 rounded-lg bg-panel-2 border border-border text-xs hover:border-accent/30 transition-colors"
                      >
                        <div class="flex items-start gap-2">
                          ...
                          <a
                            ...
                            class="font-medium text-tx hover:text-accent transition-colors flex-1"
                            >{{ n.title }}</a
                          >
                        </div>
                        <div class="text-xs text-muted mt-1 ml-5">
              ...
              <div
                class="p-3 rounded-lg bg-panel-2 border border-border text-xs text-muted leading-relaxed"
              >
                {{ d.disclaimer }}
              </div>
            </div>
