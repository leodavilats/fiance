...
- Testes: `cd backend && python -m pytest -q` (215 testes) e `python -m ruff check app tests migrations`. Mobile: `flutter analyze && flutter test` (22 testes). Web: `npm run format:check && npm run build` — **confira o código de saída**: o build imprime os erros como `X [ERROR] TS…`, então um `grep -i error` ingênuo passa reto por eles. Tudo roda no CI (`.github/workflows/ci.yml`) em todo push — mudança que quebra a suíte não deve ser mergeada.
