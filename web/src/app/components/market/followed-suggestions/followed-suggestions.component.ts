import { CommonModule } from '@angular/common';
import { ProvenanceComponent } from '../../provenance/provenance.component';
import { Component, inject, OnInit, signal } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { LucideAngularModule } from 'lucide-angular';
import {
  FollowedSuggestionPayload,
  FollowedSuggestionsResponse,
  RecommendService,
  SnackbarService,
} from '../../../core';

@Component({
  selector: 'app-followed-suggestions',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule, ProvenanceComponent],
  template: `
    @if (data(); as d) {
      <div class="fi-block">
        <div class="flex flex-wrap items-start justify-between gap-3 mb-1">
          <h2 class="fi-title m-0 text-ink">Sugestões que você seguiu</h2>
          <button type="button" class="btn-secondary" (click)="showForm.set(!showForm())">
            <lucide-icon [name]="showForm() ? 'x' : 'plus'" size="14"></lucide-icon>
            {{ showForm() ? 'Cancelar' : 'Registrar execução' }}
          </button>
        </div>
        <p class="fi-body text-ink-2 mb-4">
          Registre o que você de fato executou a partir de uma sugestão do app. É assim que o
          produto fica auditável por você: o resultado abaixo é histórico verificável, não promessa.
        </p>

        @if (showForm()) {
          <div class="card mb-5">
            <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:items-end">
              <label class="flex flex-col gap-1">
                <span class="fi-label text-ink-2">Ativo</span>
                <input
                  type="text"
                  [ngModel]="form().ticker"
                  (ngModelChange)="updateField({ ticker: $event })"
                  placeholder="ex.: PETR4"
                  class="input uppercase"
                />
              </label>
              <label class="flex flex-col gap-1">
                <span class="fi-label text-ink-2">Veio de</span>
                <select
                  [ngModel]="form().source"
                  (ngModelChange)="updateField({ source: $event })"
                  class="input"
                >
                  @for (s of sources; track s.value) {
                    <option [value]="s.value">{{ s.label }}</option>
                  }
                </select>
              </label>
              <label class="flex flex-col gap-1">
                <span class="fi-label text-ink-2">Quantidade</span>
                <input
                  type="number"
                  min="0"
                  step="any"
                  [ngModel]="form().quantity"
                  (ngModelChange)="updateField({ quantity: $event })"
                  class="input"
                />
              </label>
              <label class="flex flex-col gap-1">
                <span class="fi-label text-ink-2">Preço executado (R$)</span>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  [ngModel]="form().price"
                  (ngModelChange)="updateField({ price: $event })"
                  class="input"
                />
              </label>
              <label class="flex flex-col gap-1">
                <span class="fi-label text-ink-2">Data</span>
                <input
                  type="date"
                  [ngModel]="form().followed_on"
                  (ngModelChange)="updateField({ followed_on: $event })"
                  class="input"
                />
              </label>
            </div>

            <button type="button" class="btn-primary" [disabled]="saving()" (click)="save()">
              <lucide-icon name="save" size="14"></lucide-icon>
              {{ saving() ? 'Salvando…' : 'Registrar' }}
            </button>
          </div>
        }

        @if (d.items.length === 0) {
          <p class="fi-body text-ink-2 italic m-0">{{ d.summary }}</p>
        } @else {
          <div
            class="p-4 rounded-md mb-4 border"
            [class.border-favorable]="d.beat_ibov ?? false"
            [class.bg-favorable/5]="d.beat_ibov ?? false"
            [class.border-hairline]="!(d.beat_ibov ?? false)"
            [class.bg-ground-2]="!(d.beat_ibov ?? false)"
          >
            <div class="fi-label text-ink">{{ d.summary }}</div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
              <div>
                <div class="fi-eyebrow text-ink-2">Investido</div>
                <div class="fi-label text-ink">R$ {{ d.total_invested | number: '1.2-2' }}</div>
              </div>
              <div>
                <div class="fi-eyebrow text-ink-2">Valor hoje</div>
                <div class="fi-label text-ink">
                  R$ {{ d.total_current_value | number: '1.2-2' }}
                </div>
              </div>
              <div>
                <div class="fi-eyebrow text-ink-2">Resultado</div>
                <div class="fi-label text-ink">
                  {{ d.total_pnl >= 0 ? '+' : '' }}R$ {{ d.total_pnl | number: '1.2-2' }} ({{
                    d.total_pnl_pct | number: '1.2-2'
                  }}%)
                </div>
              </div>
            </div>
          </div>

          @if (d.by_source.length > 1) {
            <div class="mb-4">
              <div class="fi-eyebrow text-ink-2 mb-2">Por origem da sugestão</div>
              <div class="flex flex-col gap-2">
                @for (group of d.by_source; track group.source) {
                  <div class="fi-body flex items-center justify-between gap-3">
                    <span class="text-ink">
                      {{ group.source }}
                      <span class="fi-caption text-ink-2">({{ group.count }})</span>
                    </span>
                    <span class="flex items-center gap-3 shrink-0">
                      <span class="fi-label text-ink">
                        {{ group.pnl_pct >= 0 ? '+' : '' }}{{ group.pnl_pct | number: '1.1-1' }}%
                      </span>
                      @if (group.ibov_pct != null) {
                        <span class="fi-caption text-ink-2">
                          Ibov {{ group.ibov_pct >= 0 ? '+' : ''
                          }}{{ group.ibov_pct | number: '1.1-1' }}%
                        </span>
                      }
                    </span>
                  </div>
                }
              </div>
            </div>
          }

          <div style="overflow-x: auto">
            <table class="fi-body w-full border-collapse">
              <thead>
                <tr class="border-b border-hairline">
                  <th class="fi-label text-left py-2 px-2 text-ink-2">Ativo</th>
                  <th class="fi-label text-left py-2 px-2 text-ink-2">Origem</th>
                  <th class="fi-label text-left py-2 px-2 text-ink-2">Data</th>
                  <th class="fi-label text-right py-2 px-2 text-ink-2">Investido</th>
                  <th class="fi-label text-right py-2 px-2 text-ink-2">Hoje</th>
                  <th class="fi-label text-right py-2 px-2 text-ink-2">Resultado</th>
                  <th class="fi-label text-right py-2 px-2 text-ink-2">vs Ibov</th>
                  <th class="fi-label text-right py-2 px-2 text-ink-2"></th>
                </tr>
              </thead>
              <tbody>
                @for (item of d.items; track item.id) {
                  <tr class="border-b border-hairline hover:bg-ground-2 transition-colors">
                    <td class="py-2 px-2">
                      <div class="fi-label text-ink">{{ item.ticker }}</div>
                      @if (item.score_at_suggestion != null) {
                        <div class="fi-caption text-ink-2">
                          score {{ item.score_at_suggestion | number: '1.0-0' }} na sugestão
                        </div>
                      }
                    </td>
                    <td class="py-2 px-2 text-ink-2">{{ sourceLabel(item.source) }}</td>
                    <td class="py-2 px-2 text-ink">
                      {{ item.followed_on }}
                      <div class="fi-caption text-ink-2">{{ item.days_held }} dias</div>
                    </td>
                    <td class="text-right py-2 px-2 text-ink">
                      {{ item.invested | number: '1.2-2' }}
                    </td>
                    <td class="text-right py-2 px-2 text-ink">
                      {{
                        item.current_value != null ? (item.current_value | number: '1.2-2') : '—'
                      }}
                    </td>
                    <td
                      class="fi-label text-right py-2 px-2"
                      [class.text-up]="(item.pnl_pct ?? 0) >= 0"
                      [class.text-down]="(item.pnl_pct ?? 0) < 0"
                    >
                      {{ item.pnl_pct != null ? (item.pnl_pct | number: '1.2-2') + '%' : '—' }}
                    </td>
                    <td class="text-right py-2 px-2">
                      @if (item.ibov_pct_since != null) {
                        <span
                          [class.text-favorable]="item.beat_ibov"
                          [class.text-ink-2]="!item.beat_ibov"
                        >
                          {{ item.ibov_pct_since >= 0 ? '+' : ''
                          }}{{ item.ibov_pct_since | number: '1.1-1' }}%
                        </span>
                      } @else {
                        <span class="text-ink-2">—</span>
                      }
                    </td>
                    <td class="text-right py-2 px-2">
                      <button
                        type="button"
                        class="btn-icon btn-icon-danger"
                        title="Remover registro"
                        (click)="remove(item.id)"
                        aria-label="Remover sugestão seguida"
                      >
                        <lucide-icon name="trash-2" size="14"></lucide-icon>
                      </button>
                    </td>
                  </tr>
                }
              </tbody>
            </table>
          </div>
        }
      </div>
    }

    <app-provenance
      method="Compara o preço de hoje com o do dia em que você registrou ter seguido a sugestão, e a mesma janela no Ibovespa."
      source="Preços da BRAPI; as sugestões que você marcou como seguidas."
      limitation="Só entra o que você registrou. Se sobrarem sobretudo os acertos, o resultado fica otimista: o número mede o que foi anotado, não o que aconteceu."
    ></app-provenance>
  `,
})
export class FollowedSuggestionsComponent implements OnInit {
  private readonly svc = inject(RecommendService);
  private readonly snackbar = inject(SnackbarService);

  data = signal<FollowedSuggestionsResponse | null>(null);
  showForm = signal(false);
  saving = signal(false);

  form = signal<FollowedSuggestionPayload>({
    ticker: '',
    source: 'opportunities',
    action: 'comprar',
    quantity: 0,
    price: 0,
    followed_on: new Date().toISOString().slice(0, 10),
  });

  readonly sources = [
    { value: 'opportunities', label: 'Oportunidades' },
    { value: 'rebalance', label: 'Sugestões de ajuste' },
    { value: 'quick_invest', label: 'Quick Invest' },
    { value: 'strategy', label: 'Estratégia' },
    { value: 'dip_scanner', label: 'Scanner de quedas' },
    { value: 'whats_new', label: 'O que mudou' },
  ];

  ngOnInit(): void {
    this.load();
  }

  load(): void {
    this.svc.getFollowedSuggestions().subscribe({
      next: res => this.data.set(res),
      error: () => {},
    });
  }

  updateField(patch: Partial<FollowedSuggestionPayload>): void {
    this.form.update(current => ({ ...current, ...patch }));
  }

  save(): void {
    const form = this.form();
    if (!form.ticker.trim() || form.quantity <= 0 || form.price <= 0) {
      this.snackbar.showError('Informe ticker, quantidade e preço executado.');
      return;
    }

    this.saving.set(true);
    this.svc
      .registerFollowedSuggestion({ ...form, ticker: form.ticker.trim().toUpperCase() })
      .subscribe({
        next: () => {
          this.saving.set(false);
          this.snackbar.showSuccess('Registrado. O resultado aparece aqui a partir de agora.');
          this.form.set({
            ticker: '',
            source: 'opportunities',
            action: 'comprar',
            quantity: 0,
            price: 0,
            followed_on: new Date().toISOString().slice(0, 10),
          });
          this.showForm.set(false);
          this.load();
        },
        error: err => {
          this.saving.set(false);
          this.snackbar.showError(err?.error?.detail || 'Não foi possível registrar.');
        },
      });
  }

  remove(id: number): void {
    this.svc.deleteFollowedSuggestion(id).subscribe({
      next: () => this.load(),
      error: () => this.snackbar.showError('Não foi possível remover o registro.'),
    });
  }

  sourceLabel(source: string): string {
    return this.sources.find(s => s.value === source)?.label ?? source;
  }
}
