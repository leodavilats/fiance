...
export class DesempenhoComponent implements OnInit {
  ...
  /**
   * O que os dois retratos dizem, em uma frase.
   *
   * A tela perguntava "Como meu patrimônio evoluiu?" num eyebrow e respondia
   * com uma curva. A resposta estava em memória o tempo todo.
   */
  ...
  /** A comparação com o CDI, dita — não deixada para o olho derivar da curva. */
  ...
  /**
   * De onde veio a taxa que desenhou a curva.
   *
   * O backend sempre mandou `cdi_source`, e o TypeScript não o declarava: a
   * curva era desenhada sem dizer se veio do Banco Central, de um cache
   * vencido ou de um valor de reserva. `cdi_basis` diz que a curva extrapola a
   * taxa de hoje para trás — é referência, não o CDI acumulado histórico.
   */
