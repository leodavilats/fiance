...
def list_snapshots(
    ...
    limit: int = 90, user_id: str = DEFAULT_USER
...
def last_updated(user_id: str = DEFAULT_USER) -> Optional[float]:
...
def list_goals(user_id: str = DEFAULT_USER) -> List[Goal]:
    ...
    with _conn() as cx:
        ...
        rows = cx.execute(
            ...
            "SELECT category, target_pct FROM goals WHERE user_id = ? ORDER BY category",

            (user_id,),

        ).fetchall()

    return [Goal(category=r["category"], target_pct=r["target_pct"]) for r in rows]
...
def replace_goals(goals: List[Goal], user_id: str = DEFAULT_USER) -> None:
    ...
    with _conn() as cx:
        ...
        cx.executemany(
            ...
            "INSERT INTO goals(user_id, category, target_pct) VALUES (?, ?, ?)",

            [(user_id, g["category"], float(g["target_pct"])) for g in goals],
