...
Esta migracao substitui as doze que existiam antes. O historico foi colapsado
em 2026-08-28, quando o banco ainda nao tinha titular de verdade: nenhuma delas
descrevia uma transicao que algum banco vivo fosse fazer, e duas carregavam
migracao de **dado** sobre um passado que deixou de existir — a limpeza do
ticker sintetico `RF_*`, que o produto abandonou, e o backfill de
`taxable_profit`.

Colapsar e diferente de abandonar. O Alembic fica: a partir do primeiro
assinante, resetar deixa de ser opcao, e reintroduzir versionamento de schema
sob pressao e como isso e feito mal. O que muda e que a linha do tempo comeca
aqui, com o esquema que os modelos descrevem hoje.

O que impede esta migracao de divergir dos modelos nao e disciplina: e
`test_database_migration.py`, que sobe um banco por esta cadeia, outro por
`Base.metadata`, e compara tabela por tabela e coluna por coluna.
"""
